...
by the NN. The first 70% of data is still fed shuffled during training in order to prevent
...
def run():
    ...
    train_n128, validation_n128 = Subset(data_n128, range(train_ind)), Subset(data_n128, range(train_ind, int(len(data_n128) * 0.9)))
    ...
    models_n128 = []
    for r in ranks_n128:
        m = ConvMF(r, mat_size_2, conv_dims=conv_dims).to(device)
