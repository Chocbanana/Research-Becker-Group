...
for r_ind = 1:1000
    ...
    data_out_path = ['..\..\data\gen_plasma_n128\rand_', num2str(r_ind), '\']; % PC Path
    ...
    if prob == 6
        T = 1000*dt;
        disp('Warning, only running simulation for 1000 time steps')
    end
    ...
    for i = 4:Nt
        ...
        % NOTE
        % BJ: Save Values
        % TODO: save(filename,variables,version,"-nocompression") saves the variables to the MAT-file without compression. The "-nocompression" option supports only MAT-file Version 7 (default) and Version 7.3. Therefore, you must specify version as "-v7" or "-v7.3". The variables argument is optional
        full_f = full(f);
        fnorm = norm(full_f, "fro");
        filename = [data_out_path, num2str(i-4), '.hdf5'];
        h5create(filename,"/f", size(full_f))
        h5write(filename,"/f", full_f)
    
        h5create(filename,"/U", size(f.U{2}))
        h5write(filename,"/U", f.U{2})

        h5create(filename,"/V", size(f.U{3}))
        h5write(filename,"/V", f.U{3})

        h5create(filename,"/S", size(f.B{1}))
        h5write(filename,"/S", f.B{1})
